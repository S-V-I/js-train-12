 ...
 * Ключами об'єкту стають ключі словника, а значеннями об'єкту -
 * значення відповідних ключів у словнику.
 *
...
// Завдання: 5
/**
 * Функція `getKeysStartingWith` повертає масив ключів словника, які починаються
 * з заданої букви.
 ...
 * Повертаємо - Об'єкт, що містить змінений словник, кількість доданих
 * та відхилених ключів.
 */
function addKeyValuePairs(dictionary, entries) {
  // Ведемо статистику доданих та відхилених ключів, для цього створимо
  // змінні added та rejected з початковими значеннями 0
  ...
  // Якщо ключ є унікальним, додаємо його до словника за допомогою методу set
  // та збільшимо added на 1
...
// Завдання: 7
/**
 * Функція `transformDictionary` перетворює словник в об'єкт, який містить
 * масиви ключів, значень і пар ключ-значення.
...
function transformDictionary(dictionary) {
  ...
  // Використовуємо цикл for...of і метод entries для перебору пар ключ-значення
  // словника
 ...
 * filter - Фільтрувальна функція. Ця функція приймає пару ключ-значення та
 * повертає true, якщо пара відповідає фільтру, або false в іншому випадку.
...
function getFilteredDictionarySize(dictionary, filter) {
  ...
  // Використовуємо for...of цикл разом з методом entries() для перебору
  // пар [ключ, значення] словника
  // Якщо пара [ключ, значення] відповідає фільтру, додаємо її
  // до фільтрованого словника
...
function sortByValues(dictionary) {
  // Конвертуємо словник в масив пар ключ-значення за допомогою оператора
  // деструктурізації
  ...
  console.log(entry.sort((a, b) => b[1] - a[1]));
  const sortedDictionary = new Map(entry);
...
function resetDictionary(dictionary, maxSize) {
  ...
  // Якщо кількість елементів більша ніж максимально допустима, очищуємо
  // словник за допомогою методу clear
  // Повертаємо true, оскільки словник був очищений
  // Повертаємо false, оскільки кількість елементів не перевищує максимально
  // допустиму, тому словник не був очищений
 ...
 * Значенням множини є ключі словника, де рядкові ключі переводяться в
 * верхній регістр,
 ...
 * Ключами словника стають елементи множини, а значеннями - коди перших символів
 * цих елементів.
 *
 * set - Множина, яку потрібно перетворити.
 *
 * Повертає - Словник, який містить елементи множини як ключі та їх коди
 * перших символів як значення.
 */
